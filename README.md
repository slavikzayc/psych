...
Проект реализован на C++17 для операционной системы Windows. Отображение выполнено средствами консольной ASCII-графики. Игровые сущности реализованы не через иерархию игровых объектов, а через ECS-подобную модель `Entity + Components + Systems`.
...
Игровые объекты не представлены классами `Player`, `Patient`, `Item` или `Door`. Вместо этого каждый объект является числовой сущностью типа `Entity`. Поведение объекта определяется набором компонентов, хранящихся в `Registry`. Игровая логика вынесена в отдельные системы.

Такой подход позволяет отделить данные игровых объектов от алгоритмов их обработки и избежать глубокой иерархии наследования.
