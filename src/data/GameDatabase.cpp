...
bool GameDatabase::LoadAll(const std::string& assets_path) {
  ...
  try {
    config.starting_map_id = root.value("starting_map_id", config.starting_map_id);
    ...
    config.stat_allocation.start_authority =
        stat_config.value("start_authority", config.stat_allocation.start_authority);
    config.stat_allocation.start_medication =
        stat_config.value("start_medication", config.stat_allocation.start_medication);
    ...
    config.sanity.initial = sanity_config.value("initial", config.sanity.initial);
    config.sanity.max = sanity_config.value("max", config.sanity.max);
    config.sanity.passive_drain_per_minute =
        sanity_config.value("passive_drain_per_minute", config.sanity.passive_drain_per_minute);
    config.sanity.passive_drain_interval_seconds = sanity_config.value(
        "passive_drain_interval_seconds", config.sanity.passive_drain_interval_seconds);
    ...
    for (const json& patient_value : root) {
      ...
      patient.dialogue_cooldown_seconds =
          patient_value.value("dialogue_cooldown_seconds", patient.dialogue_cooldown_seconds);
