...
std::string FormatTime(int seconds) {
  ...
  out << std::setfill('0') << std::setw(2) << (seconds / 60) << ":" << std::setw(2)
      << (seconds % 60);
  return out.str();
}
}

std::string HUDSystem::BuildHud(const WorldState& world, const std::string& prompt) const {
  std::ostringstream out;
  const auto sanity_it = world.registry.sanities.find(world.player);
  if (sanity_it != world.registry.sanities.end()) {
    out << "\nРассудок: " << sanity_it->second.current << " / " << sanity_it->second.max;
  }
  out << "\nВремя смены: " << FormatTime(world.shift_time_seconds) << " / "
      << FormatTime(world.shift_duration_seconds);
  out << "\nФаза: " << PhaseName(world.phase);
  out << "\nСообщение: "
      << (world.message.empty() ? "Тишина давит сильнее шума."
                                : world.message);
  if (!prompt.empty()) {
    out << "\nF - " << prompt;
  }
